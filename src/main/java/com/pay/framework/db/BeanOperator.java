...
import com.pay.framework.annotation.Param;
...
public class BeanOperator{
	...
	public static <T> String getSqlByObject(SqlTypes sqlType , T obj , String tableName){
		Class<?> clz = obj.getClass();
		String[] intTypes = {"int","java.lang.Integer","short","java.lang.Short","long","java.lang.Long"};
		String[] dateFields = {"time","date"};
		...
		if(sqlType == SqlTypes.INSERT){
			...
			for(Field field : fields){
				...
				if (StringUtil.contains(intTypes, type.getName())){
						isNumber = true;
				} else if (StringUtil.containsItemContains(dateFields, fieldName.toLowerCase())){//含有 date 或 time 的字段为日期字段
					isDate = true;
				}
				...
				if (index == 1){
					...
					if(isNumber){
						...
						values.append("date_format('"+value+"','%Y-%c-%d %H:%i:%s')");
					} else {
	...
	public static void main(String[] args) {
		...
		map.put("corderid", "aaa");
		map.put("username", "bbb");

		List<Map<String,Object>> list = new ArrayList<Map<String,Object>>();
		Map<String,Object> map1 = new HashMap<String,Object>();
		map1.put("chargeamount", "ddd");
		map1.put("submitdate", "kkk");
		list.add(map1);
		list.add(map);
		OrderFormUnPay form = new OrderFormUnPay();
		form.setCorderid("youxing");
		try {
			System.out.println(PropertyUtils.getPropertyType(form, "productid"));
		} catch (IllegalAccessException e) {
			e.printStackTrace();
