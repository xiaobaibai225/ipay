...
public class AlipayNotify {
	...
	private static LogManager logManager = LogManager.getLogger(AlipayNotify.class);
    ...
    /**
     * 验证消息是否是支付宝wap发出的合法消息
     * @param params 通知返回来的参数数组
     * @return 验证结果
     */
    public static boolean verifyWap(Map<String, String> params,String partner,String key) {
	    ...
	    String sign = "";
	    if(params.get("sign") != null) {sign = params.get("sign");}
	    boolean isSign = getWapSignVeryfy(params, sign, key);
     ...
     * @return 生成的签名结果
     */
	private static boolean getWapSignVeryfy(Map<String, String> Params, String sign,String key) {
    ...
    */
    private static String checkUrl(String urlvalue) {
        ...
        try {
        	logManager.debug("验证支付宝通知", "第一次握手"+urlvalue);
            ...
            e.printStackTrace();
        	logManager.debug("验证支付宝通知", "第二次握手"+urlvalue);
        	try{
            URL url = new URL(urlvalue);
            HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
            BufferedReader in = new BufferedReader(new InputStreamReader(urlConnection
        	...
        	{
        		 e1.printStackTrace();
             	logManager.debug("验证支付宝通知", "第三次握手"+urlvalue);
            	try{
                URL url = new URL(urlvalue);
                HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
                BufferedReader in = new BufferedReader(new InputStreamReader(urlConnection
            	...
            	{
            		 e2.printStackTrace();
                  	logManager.debug("验证支付宝通知", "第四次握手"+urlvalue);
                	try{
                    URL url = new URL(urlvalue);
                    HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
                    BufferedReader in = new BufferedReader(new InputStreamReader(urlConnection
