 ...
 */
@Component("alipay")
public class Alipay extends BasePay implements IPay, IMobilePay, ICodePay {
	...
	public String getForwardUrl(HttpServletRequest request, HttpServletResponse response, int payTypeId) {
		...
		/**
		 * 此处上线的时候再确认是否哪些项目或公司账号支持信用卡支付
		 */
//		if (orderform.getCompanyid() == 2) {
//			sParaTemp.put("credit_card_pay", "Y");
//			sParaTemp.put("credit_card_default_display", "Y");
//		}
		String isquick = request.getParameter("isquick");
		String defaultbank = orderform.getDefaultbank();
		if (isquick != null && isquick.equals("1")) {
			sParaTemp.put("default_login", "Y");
			sParaTemp.put("paymethod", "motoPay");
			sParaTemp.put("defaultbank", defaultbank);
		} else {
			if (orderform.getDefaultbank() != null && !(orderform.getDefaultbank().trim().equals(""))) {
				// 默认支付方式
		...
		String bigpay = request.getParameter("bigpay");
		if (StringUtils.isNotEmpty(bigpay) && bigpay.toUpperCase().equals("Y")) {
			sParaTemp.put("credit_card_pay", "Y");
			sParaTemp.put("credit_card_default_display", "Y");
			sParaTemp.put("paymethod", "CREDITCARD");
			sParaTemp.put("defaultbank", defaultbank.toLowerCase());
		}
		// 建立请求
		String sHtmlText = AlipaySubmit.buildForwardUrl(payment.getPosturl(),sParaTemp, payment.getSeckey());
		return sHtmlText;
	}
	...
	public int doPaymentServerNotify(HttpServletRequest request, HttpServletResponse response) {
		...
		if (AlipayNotify.verify(parameterMap, payment.getMerchantid(), payment.getSeckey())) {// 验证成功
