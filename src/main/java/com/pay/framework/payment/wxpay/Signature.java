...
public class Signature {
	...
	static LogManager logManager = LogManager.getLogger(Signature.class);
    /**
     * 签名算法
     * @param o 要参与签名的数据对象
     * @return 签名
    ...
    public static String getSign(Object o,String key) throws IllegalAccessException {
        ...
        result += "key=" +key;
        logManager.debug("Sign Before MD5:" , result);
        result = MD5.MD5Encode(result).toUpperCase();
        logManager.debug("Sign Result:" , result);
        return result;
    }
    ...
    public static boolean checkIsSignValidFromResponseString(String responseString,String key) throws ParserConfigurationException, IOException, SAXException {
        ...
        Map<String,Object> map = XMLParser.getMapFromXML(responseString);
        logManager.debug("map to string is",map.toString());

        String signFromAPIResponse = map.get("sign").toString();
        if(signFromAPIResponse=="" || signFromAPIResponse == null){
            logManager.debug("API返回的数据签名数据不存在，有可能被第三方篡改!!!",null);
            return false;
        }
        logManager.debug("服务器回包里面的签名是:" , signFromAPIResponse);
        //清掉返回数据对象里面的Sign数据（不能把这个数据也加进去进行签名），然后用签名算法进行签名
        map.put("sign","");
        //将API返回的数据根据用签名算法进行计算新的签名，用来跟API返回的签名进行比较
        ...
        if(!signForAPIResponse.equals(signFromAPIResponse)){
            ...
            logManager.debug("API返回的数据签名验证不通过，有可能被第三方篡改!!!",null);
            return false;
        }
        logManager.debug("恭喜，API返回的数据签名验证通过!!!",null);
        return true;
    }
}